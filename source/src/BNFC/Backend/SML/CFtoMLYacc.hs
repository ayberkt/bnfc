...
module BNFC.Backend.SML.CFtoMLYacc
       (
...
header _ absName _ cf = unlines
         ["/* This ML-Yacc file was machine-generated by the BNF converter */",
          "%{",
          "open " ++ absName,
          "open Lexing",
          "",
...
terminal cf s  = case lookup s (zip (cfgSymbols cf) [1..]) of
    Just i -> "SYMB" ++ show i
    Nothing -> error $ "CFtoMLYacc: terminal " ++ show s ++ " not defined in CF."
...
specialRules cf = unlines $
 ...
 where
   aux cat =
     case cat of
         TokenCat "Ident"   -> "ident : TOK_Ident ( Ident TOK_Ident );"
         TokenCat "String"  -> "string : TOK_String ( TOK_String );"
         TokenCat "Integer" -> "int :  TOK_Integer ( TOK_Integer );"
         TokenCat "Double"  -> "float : TOK_Double ( TOK_Double );"
         TokenCat "Char"    -> "char : TOK_Char ( TOK_Char );"
